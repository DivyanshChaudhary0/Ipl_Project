...
function Home() {
  ...
  async function fetchData() {
    ...
    try {
      ...
      // console.log(result.matchDetails);
      const filteredData = result.matchDetails.filter(
  ...
  return (
    <>
    {
      matchData.length <= 0?<div className="text-white">Loading...</div>:<div className=" w-[100%] mx-auto min-h-screen bg-black py-8 flex flex-wrap gap-4 items-start justify-center opacity-80">
      {matchData.map(function (data, i) {
        return data.matchDetailsMap.match.map((match, idx) => {
          return (
            <NavLink to={`/matchDetails/${match.matchInfo.matchId}`} key={idx}>
            <div key={idx}
            className="card w-[400px] h-[250px] text-white border border-solid border-slate-500 px-6 py-4 rounded-md bg-[#3d3f43be] hover:bg-[#373839be]">

              <div className="nav flex justify-between py-1">
                <p className="para flex gap-2">
                  <span>{match.matchInfo.matchFormat}</span>
                  <span>{match.matchInfo.matchDesc}</span>
                </p>
                <p>{data.matchDetailsMap.key}</p>
              </div>

              <div className="team1 flex justify-between py-1">
                <p className="para flex gap-3 items-center ">
                    <span>
                        <img className="w-6"
                        src={`https://res.cloudinary.com/digkgdovw/image/upload/v1715267905/iplTeamLogo/${match.matchInfo.team1.imageId}`} alt="" />
                    </span>
                    <span>{match.matchInfo.team1.teamSName}</span>
                </p>

                <p>
                    {
                        match.matchInfo.state==="Complete"?`${match.matchScore.team1Score.inngs1.runs}
                        /${match.matchScore.team1Score.inngs1.wickets} 
                        (${match.matchScore.team1Score.inngs1.overs})`:"Upcoming"
                    }
                </p>
              </div>

              <div className="team2 flex justify-between py-1">
              <p className="para flex gap-3 items-center">
                    <span>
                        <img className="w-6 object-cover"
                        src={`https://res.cloudinary.com/digkgdovw/image/upload/v1715267905/iplTeamLogo/${match.matchInfo.team2.imageId}`} alt="" />
                    </span>
                    <span>{match.matchInfo.team2.teamSName}</span>
                </p>

                <p>
                    {
                        match.matchInfo.state==="Complete"?`${match.matchScore.team2Score.inngs1.runs}
                        /${match.matchScore.team2Score.inngs1.wickets} 
                        (${match.matchScore.team2Score.inngs1.overs})`:"Upcoming"
                    }
                </p>
              </div>

              <div className="place py-1">
                <p className="city py-3">City: {match.matchInfo.venueInfo.city}</p>
                <p className="ground">Ground: {match.matchInfo.venueInfo.ground}</p>
              </div>
